...
class TestEventSerializer:
    def test_valid_serializer(self):
        ...
        data = {'sensor': sensor.id, 'name': 'Test Event', 'temperature': 25.5, 'humidity': 60.0}
        serializer = EventSerializer(data=data)
        assert serializer.is_valid()

    def test_event_without_sensor(self):
        data = {'name': 'Test Event', 'temperature': 25.5}
        serializer = EventSerializer(data=data)
        assert not serializer.is_valid()
    ...
    def test_event_temperature_validation(self):
        ...
        sensor = Sensor.objects.create(name='Test Sensor', sensor_type=Sensor.SensorType.TYPE_1)
        data = {'sensor': sensor.id, 'name': 'Test Event', 'temperature': 'invalid'}
        serializer = EventSerializer(data=data)
        assert not serializer.is_valid()
